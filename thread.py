...
def show_thread():
    ...
    for row in result:
        print threads.substitute(thread_id = row[0], thread_name = row[1].encode("utf-8"), creator_name = row[2].encode("utf-8"), comment_number = str(row[3]), update_day = str(row[4]))

threads = Template("""
<tr><td><table><tr><td width=\"300\"><a href=\"comment.py?thread=${thread_id}\">${thread_name}</a></td><td width=\"200\">${creator_name}</td><td width=\"100\"><div align=\"center\">${comment_number}</div></td$
""")
...
<tr><td><table><tr><td width=\"300\">スレッド名<td><td width=\"200\">制作者</td><td width=\"100\"><div align=\"center\">コメント数</div></td><td width=\"200\"><div align=\"center\">最終更新時間</div></td></tr$
""")
